...
summary:
implements multiplication of matrices up to 2048 x 2048 on the webgl by
encoding matrices as 32-bit floats in the Red and Green planes of an
RGB texture.
also supports 2x byte textures for webgls that don't support FLOAT textures.

neat things:
1. a non pre-multiplied RGBA texture is rendered to in IEEE754-format by
the webgl code. this allows us to read floats directly by calling readPixels
...
for the zero check)
2. for webgls such as the Samsung Chromebook, which do not have oes_texture_float_extension, 
...
to munge the bytes back into floats on the webgl.
...
function webgl_matrix() {};

webgl_matrix.prototype = {

	__webglmatrix__: undefined,
	...
	_checkinit: function(r,c) {
		...
		canvas.height = r;
		canvas.width = c;
		if (this.__webglmatrix__===undefined) {
			console.log("getting webgl");
			// get webgl context
			webgl_matrix.prototype.__webglmatrix__ = canvas.getContext("experimental-webgl",
			...
			var ext;
			try {
				ext = webgl_matrix.prototype.__webglmatrix__.getExtension("OES_texture_float");
			} catch(e) {}
			if ( !ext ) {
		...
		console.log("this.__webglmatrix__ " + this.__webglmatrix__);
		webgl_matrix.prototype.__webglmatrix__.viewport(0, 0, c, r);
		return this.__webglmatrix__;
	},
	...
	_multiply: function(m1,m2) {
		...
		return webgl_matrix.create(m1.r,m2.c,new Float32Array(rawbuffer));
	},
...
webgl_matrix.create = function(r,c,data) {
	var M = new webgl_matrix();
